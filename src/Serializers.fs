...
let private smallBuffer() = ArrayPool.Shared.Rent 300
let private bigBuffer() = ArrayPool.Shared.Rent 30000
let private stream buffer = new MemoryStream(buffer, 0, buffer.Length, true, true)

let private utf8Encoding = Encoding.UTF8
let private utf8 : string -> byte[] = utf8Encoding.GetBytes
...
let serializeUser (user: User) : MemoryStream =
    ...
    write ``,"first_name":"``
    write(utf8 user.first_name)
    write ``","last_name":"``
    write(utf8 user.last_name)
    write ``","gender":"``
    write(utf8(string user.gender))
    write ``","email":"``
    write(utf8 user.email)
    write ``"}``
    output
